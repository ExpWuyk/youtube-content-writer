...
1. 下载 [Releases](https://github.com/ExpWuyk/youtube-content-writer/releases) 中的 `youtube-content-writer.skill` 文件
2. 在 Claude Code 中运行：
   ```
...
2. 将 `skill/` 目录复制到 Claude Code 技能目录：
   ```bash
   # Windows
...
### 方式二：直接发送链接
直接在 Claude Code 中发送 YouTube 链接，skill 会自动识别并触发。

### 方式三：带提示词文件
```
https://www.youtube.com/watch?v=VIDEO_ID 按照要求 D:\视频\视频内容提示词.md
```
